...
func FlagMigrations(path string) (string, error) {
	return migrateFromDir(path)
}
var runMigrationsHandler = func(migration.RunMigrationsParams) middleware.Responder {
	logrus.Info("Running migrations")
	migrationPath, hasMigrationPath := os.LookupEnv("FLAGR_MIGRATION_PATH")
	if !hasMigrationPath {
		migrationPath = "migrations"
	}
	status, err := migrateFromDir(migrationPath)
	if err != nil {
		status = fmt.Sprintf("error running migrations: %v", err);
		return migration.NewRunMigrationsDefault(500).WithPayload(&models.MigrationStatus{Message: status, Code: 500})
	}

	tx := getDB()

	var names []string
	tx.Model(&entity.FlagMigration{}).Pluck("name", &names)

	return migration.NewRunMigrationsOK().WithPayload(&models.MigrationStatus{Message: status, Migrations: names, Code: 200})
}

func migrateFromDir(dir string) (string, error) {
	tx := getDB()
	fms := []entity.FlagMigration{}

	files, err := os.ReadDir(dir)
	if err != nil {
		logrus.WithField("err", err).Errorf("cannot read directory for migrations: %v %v", dir, err)
		return "", err
	}
	...
	return fmt.Sprintf("%d new migrations completed", completed),  nil
}
